...
author: Oleg (https://github.com/Oxxxyde)
"""
...
def get_bot_id(botname='Jarvis'):
	"""
	Get BOT_ID using botname and set in the env variable BOT_ID
	"""
	if environ.get('Jarvis'):
		botname = environ.get('Jarvis')
	else:
		print 'Jarvis not set in the environment'
		return None
	...
	if environ.get('SLACK_BOT_ID'):
		...
		if user_list:
			...
			print 'Jarvis not present in user list'
			return None
